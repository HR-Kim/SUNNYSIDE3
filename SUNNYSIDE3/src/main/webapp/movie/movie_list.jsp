...
<%
	...
	String url = request.getContextPath()+"/movie/do_retrieve.do";
						...
						<c:when test="${list.size()>0}">
							<c:forEach var="vo" items="${list}">
								<tr>
									...
									<td class="text-center"><input type="button" id="doSave" name="doSave" class="btn btn-default btn-sm doSave" value="등록" /></td>
								</tr>
							</c:forEach>        			
						</c:when>        	
						<c:otherwise>
...
// 			var frm = document.frm;
// 			frm.boardId.value=movieId;
// 			frm.action = "${context}/movie/do_selectOne.do";
// 			frm.submit();
		});
		...
		$('.doSave').click(function(event) {
			...
			frm.action = "${context}/planed/do_update_planedUp.do";
			frm.submit();
		});
